      ...
      <div className="max-w-[1440px] mx-auto px-6 h-[64px] flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden lg:flex gap-1 text-[13px] font-medium">
            ...
            <a href={`/${locale}/eed`} className="px-3 py-2 hover:bg-[#F3F3F3] rounded">EED</a>
            <a href={`/${locale}#twin`} className="px-3 py-2 hover:bg-[#F3F3F3] rounded">{t('twin')}</a>
        ...
        <div className="flex gap-2 items-center">
          ...
          <a href={`/${locale}/eed`} className="px-4 py-2 border rounded text-[13px] hidden md:block">Tableau EED</a>
          <a href={`/${locale}/modules/visualisation-3d`} className="px-4 py-2 bg-[#0176D3] text-white rounded text-[13px] font-semibold">{t('demo')}</a>
        </div>
      </div>
      <div className="bg-[#032D60] text-white text-[11px]"><div className="max-w-[1440px] mx-auto px-6 py-1.5 flex justify-between"><span>QINODE.EU • SOUVERAIN • EU • AIR-GAPPED</span><span className="opacity-70">{process.env.NEXT_PUBLIC_GRAPHQL_URL || 'http://localhost:4000/graphql'}</span></div></div>
    </header>
  )
}
